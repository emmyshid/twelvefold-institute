...
// ════════════════════════════════════════════════════════════════
// TWELVEFOLD INSTITUTE — Phase 1 Homepage (v2, elevated + responsive)
// Locked identity kept (indigo / purple-gold / Space Mono + Crimson / glass).
// Signature: the celestial PhaseRing — a luminous arc sweeping the cycle.
// Depth: faint starfield + film grain. Fully responsive via media queries.
// ════════════════════════════════════════════════════════════════
...
function Starfield() {
  ...
  return (
    <div aria-hidden style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none" }}>
      {stars.map((s, i) => (
        <span key={i} className="pi-star" style={{
...
function PhaseRing() {
  ...
  const size = 460, c = size / 2, r = 145, rLabel = 186;
  const arcFrac = (active + 1) / 12;
  ...
  return (
    <div style={{ position: "relative", width: "min(440px, 86vw)", aspectRatio: "1 / 1", margin: "0 auto" }}>
      <svg viewBox={`0 0 ${size} ${size}`} width="100%" height="100%" style={{ overflow: "visible" }}>
        ...
        <circle cx={c} cy={c} r={r + 4} fill="url(#coreGlow)" />
        <circle className="pi-rot-slow" cx={c} cy={c} r={r + 24} fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="1" strokeDasharray="2 10" style={{ transformOrigin: "center" }} />
        <circle cx={c} cy={c} r={r} fill="none" stroke="rgba(255,255,255,0.08)" strokeWidth="1" />

        <circle
          cx={c} cy={c} r={r} fill="none" stroke="url(#arcGrad)" strokeWidth="2.5"
          strokeLinecap="round" pathLength="1"
          strokeDasharray={`${arcFrac} 1`} strokeDashoffset="0"
          transform={`rotate(-90 ${c} ${c})`} filter="url(#soft)"
          style={{ transition: "stroke-dasharray 1.1s " + T.ease }}
        />
          ...
          return (
            <g key={name}>
              ...
              <text x={lx} y={ly} textAnchor="middle" dominantBaseline="middle"
                ...
                letterSpacing="1.5"
                fill={on ? T.gold : "rgba(237,233,245,0.28)"}
                style={{ transition: "fill 0.7s " + T.ease, textTransform: "uppercase" }}>
                {name}
              </text>
            </g>
          );
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", pointerEvents: "none" }}>
        ...
        <div style={{ fontFamily: T.font, fontSize: "clamp(26px, 6vw, 36px)", color: T.text, fontStyle: "italic", lineHeight: 1.05, margin: "6px 0", transition: "all 0.5s " + T.ease }}>{PHASES[active]}</div>
...
// ─── UI atoms ────────────────────────────────────────────────
function Btn({ children, variant = "primary", onClick, style }: any) {
  const v: any = {
  ...
  return (
    <button onClick={onClick} style={{
      padding: "14px 30px", borderRadius: "999px", fontFamily: T.fontMono,
      fontSize: "12.5px", letterSpacing: "0.8px", cursor: "pointer",
      transition: `transform 0.3s ${T.ease}, box-shadow 0.3s ${T.ease}, border-color 0.3s ${T.ease}`,
      background: v.background, color: v.color, border: v.border, ...style,
    }}
      onMouseEnter={e => { (e.currentTarget as any).style.transform = "translateY(-2px)"; (e.currentTarget as any).style.boxShadow = `0 10px 30px ${v.glow}`; if (variant === "ghost") (e.currentTarget as any).style.borderColor = "rgba(167,139,250,0.5)"; }}
      onMouseLeave={e => { (e.currentTarget as any).style.transform = "translateY(0)"; (e.currentTarget as any).style.boxShadow = "none"; if (variant === "ghost") (e.currentTarget as any).style.borderColor = T.border; }}
    >{children}</button>
  );
}

function Eyebrow({ children, color }: any) {
  return <div style={{ display: "inline-flex", alignItems: "center", gap: "10px", fontFamily: T.fontMono, fontSize: "11px", letterSpacing: "3px", color: color || T.accent, textTransform: "uppercase", marginBottom: "18px" }}><span style={{ width: 6, height: 6, borderRadius: "50%", background: T.gold, display: "inline-block", flexShrink: 0 }} />{children}</div>;
}

// ─── Live reading (simulated demo) ──────────────────────────
function TryReading() {
  ...
  async function run() {
    ...
    try {
      ...
      setReading({ error: (e as any).message || "Something interrupted the reading." });
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div style={{ maxWidth: 700, margin: "0 auto", background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: T.radius, padding: "clamp(20px, 4vw, 30px)", backdropFilter: "blur(24px)", WebkitBackdropFilter: "blur(24px)" }}>
      ...
      <textarea value={input} onChange={e => setInput(e.target.value)} rows={3}
        placeholder="The same thing keeps showing up in my work / relationships / life…"
        style={{ width: "100%", boxSizing: "border-box", background: "rgba(255,255,255,0.05)", border: `1px solid ${T.border}`, borderRadius: T.radiusSm, color: T.text, fontFamily: T.font, fontSize: "17px", padding: "15px", resize: "vertical", outline: "none", lineHeight: 1.5 }}
      ...
      {reading && !reading.error ? (
        <div style={{ marginTop: "24px", display: "flex", flexDirection: "column", gap: "13px" }}>
          <div style={{ textAlign: "center", paddingBottom: "4px" }}>
            ...
            <div style={{ fontFamily: T.fontMono, fontSize: "11px", letterSpacing: "1px", color: T.textDim, marginTop: "9px" }}>{reading.phase} · {reading.micro_state}</div>
          </div>
          ...
          <div style={{ textAlign: "center", marginTop: "6px", fontFamily: T.font, fontStyle: "italic", fontSize: "15px", color: T.textDim }}>This is the surface. The full reading — technical depth, the six traditions, your history — lives in PatternOS.</div>
        </div>
      ) : reading?.error ? (
    ...
    <div style={{ background: T.bg, color: T.text, fontFamily: T.font, minHeight: "100vh", overflowX: "hidden", position: "relative" }}>
      <style>{`
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @keyframes pi-drift1 { 0%,100% { transform: translate(0,0) scale(1); } 50% { transform: translate(8%, 6%) scale(1.15); } }
        @keyframes pi-drift2 { 0%,100% { transform: translate(0,0) scale(1.1); } 50% { transform: translate(-7%, -5%) scale(1); } }
        @keyframes pi-pulse { 0%,100% { transform: scale(1); } 50% { transform: scale(1.025); } }
        @keyframes pi-twinkle { 0%,100% { opacity: 0.15; } 50% { opacity: 0.7; } }
        @keyframes pi-spin { to { transform: rotate(360deg); } }
        .pi-breathe { animation: pi-pulse 6s ease-in-out infinite; }
        .pi-star { animation: pi-twinkle ease-in-out infinite; }
        .pi-rot-slow { animation: pi-spin 160s linear infinite; }
        .pi-blob { position: fixed; border-radius: 50%; filter: blur(90px); opacity: 0.5; pointer-events: none; z-index: 0; }
        ...
        @media (prefers-reduced-motion: reduce) {
          .pi-breathe, .pi-star, .pi-rot-slow, .pi-blob { animation: none !important; }
        }
      ...
      <div className="pi-blob" style={{ width: 460, height: 460, top: 280, right: -130, background: "radial-gradient(circle, rgba(167,139,250,0.26), transparent 70%)", animation: "pi-drift2 28s ease-in-out infinite" }} />
      <svg className="pi-grain"><filter id="grainF"><feTurbulence type="fractalNoise" baseFrequency="0.8" numOctaves="3" stitchTiles="stitch" /></filter><rect width="100%" height="100%" filter="url(#grainF)" /></svg>
      ...
      <div style={{ position: "relative", zIndex: 3 }}>
        ...
        <nav style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "22px clamp(20px, 5vw, 64px)", gap: "16px" }}>
          ...
          <div style={{ display: "flex", gap: "20px", alignItems: "center" }}>
            <div style={{ display: "flex", gap: "28px", alignItems: "center" }} className="pi-nav-links">
              {navLinks.map(l => (
                <a key={l} href="#" style={{ fontFamily: T.fontMono, fontSize: "12px", color: T.textDim, textDecoration: "none", letterSpacing: "0.5px" }}
                  onMouseEnter={e => (e.currentTarget.style.color = T.text)} onMouseLeave={e => (e.currentTarget.style.color = T.textDim)}>{l}</a>
              ))}
            </div>
        ...
        <header style={{ padding: "clamp(36px, 7vw, 80px) clamp(20px, 5vw, 64px) clamp(20px, 5vw, 40px)", maxWidth: 980, margin: "0 auto", textAlign: "center" }}>
          ...
          <Reveal delay={0.05}>
            <h1 style={{ fontFamily: T.font, fontSize: "clamp(40px, 8vw, 78px)", lineHeight: 1.0, fontWeight: 600, letterSpacing: "-1px" }}>
              Something invisible is<br />running your life.
            </h1>
          ...
          <Reveal delay={0.12}>
            <p style={{ fontFamily: T.font, fontSize: "clamp(17px, 2.4vw, 19px)", color: T.textDim, maxWidth: 580, margin: "24px auto 0", lineHeight: 1.65 }}>
              The same situations keep returning — in work, in love, in who you become under pressure. They are not random, and they are not failure. They are curriculum. We teach you to read it.
            </p>
        ...
        <section style={{ padding: "clamp(40px, 7vw, 80px) clamp(20px, 5vw, 64px)", maxWidth: 860, margin: "0 auto", textAlign: "center" }}>
          <Reveal>
            ...
            <p style={{ fontFamily: T.font, fontSize: "clamp(17px, 2.4vw, 19px)", color: T.textDim, lineHeight: 1.7, maxWidth: 660, margin: "0 auto" }}>
              The <strong style={{ color: T.text }}>12 phases</strong> follow the solar year — twelve lunar cycles, the structure humanity has tracked for millennia. The <strong style={{ color: T.text }}>4 micro-states</strong> within each phase are our own contribution: how any cycle begins, builds, peaks, and integrates. Six wisdom traditions, working independently, named the same transformations. We translate, we do not invent.
            </p>
          </Reveal>
          <Reveal delay={0.1}>
            <div style={{ display: "flex", gap: "10px", justifyContent: "center", flexWrap: "wrap", marginTop: "34px" }}>
              {["Ifá", "Kabbalah", "I Ching", "Scripture", "Buddhism", "Hermetic"].map(t => (
                <span key={t} style={{ fontFamily: T.fontMono, fontSize: "12px", letterSpacing: "1px", padding: "10px 18px", borderRadius: "999px", border: `1px solid ${T.border}`, color: T.textDim, background: "rgba(255,255,255,0.02)" }}>{t}</span>
              ))}
            </div>
          </Reveal>
        </section>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: "18px" }}>
            {doors.map((d, i) => (
              <Reveal key={d.title} delay={i * 0.08}>
                <div className="pi-card" style={{ height: "100%", display: "flex", flexDirection: "column", padding: "28px", background: d.feature ? "rgba(167,139,250,0.07)" : T.bgCard, border: d.feature ? "1px solid rgba(167,139,250,0.3)" : `1px solid ${T.border}`, borderRadius: T.radius, backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)" }}>
                  ...
                  <Btn variant={d.variant as any}>{d.cta}</Btn>
                </div>
              </Reveal>
            ))}
          </div>
        ...
        <section style={{ padding: "clamp(40px, 7vw, 90px) clamp(20px, 5vw, 64px)", maxWidth: 820, margin: "0 auto", textAlign: "center" }}>
          <Reveal>
            ...
            <p style={{ fontFamily: T.font, fontSize: "clamp(20px, 2.8vw, 27px)", lineHeight: 1.6, color: T.text }}>
              Independent by design. Funded by its own work, never sold, never venture-backed. We hold the source traditions with respect, build authority before audience, and refuse to promise what we cannot deliver.
            </p>
        ...
        <footer style={{ borderTop: `1px solid ${T.border}`, padding: "40px clamp(20px, 5vw, 64px)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "18px" }}>
          ...
          <div style={{ display: "flex", gap: "22px", flexWrap: "wrap" }}>
            {["Pattern Literacy", "PatternOS", "Certification", "Research", "About"].map(l => (
              <a key={l} href="#" style={{ fontFamily: T.fontMono, fontSize: "12px", color: T.textMuted, textDecoration: "none" }}
                onMouseEnter={e => (e.currentTarget.style.color = T.textDim)} onMouseLeave={e => (e.currentTarget.style.color = T.textMuted)}>{l}</a>
            ))}
          </div>
        </footer>
      </div>
